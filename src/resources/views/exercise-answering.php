        ...
        <?php foreach ($exercise->questions() as $question) : ?>
            <div class="field">
                ...
                <?php if ($question->type()->slug == 'SHORT') : ?>
                    <input type="text" name="answers[<?= $question->id ?>]" id="<?= $question->id ?>" maxlength="255" value="<?= isset($submission) ? htmlspecialchars($submission->answer($question->id)->answer) : '' ?>" />
                <?php else : ?>
                    <textarea id="<?= $question->id ?>" name="answers[<?= $question->id ?>]" type="text" maxlength="255"><?= isset($submission) ? htmlspecialchars($submission->answer($question->id)->answer) : '' ?></textarea>
                <?php endif ?>
            </div>
        <?php endforeach ?>
        <div class="actions">
